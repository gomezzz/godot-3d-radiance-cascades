...
The finest 32 stored direction bins are cosine-weighted into six axes. Each lobe normalizes by its discrete sum of cosine weights so constant incident radiance is reproduced exactly. The stored quantity approximates irradiance divided by pi. Final normals use squared-component ambient-cube blending, and spatial sampling explicitly fetches eight cells without atlas bleeding. This is a low-order diffuse reconstruction, not arbitrary BRDF reconstruction.
...
1. Replace `intersect_object` / `trace_interval` with voxel DDA, a sparse SDF, or BVH traversal to support imported geometry. Preserve the radiance/transmittance contract and world-distance intervals.
...
3. Make volume, base spacing, angular basis, and level count configurable together. They currently occur in both GDScript and shader layouts; changing only one copy is invalid.
